...
class Cilindro:
    ...
    def desenha(self):
        ...
        for i in range(CONSTS.divisaoCilindro):
            for j in range(CONSTS.divisaoCilindro):
                ...
                cor = shading(v1, n1, self) * self.camisa
                glColor3f(cor.x, cor.y, cor.z)
                glTexCoord2f(*tex_coord1)
                glVertex3f(v1.x, v1.y, v1.z)
                
                cor = shading(v2, n2, self) * self.camisa
                glColor3f(cor.x, cor.y, cor.z)
                glTexCoord2f(*tex_coord2)
                glVertex3f(v2.x, v2.y, v2.z)
                
                cor = shading(v3, n3, self) * self.camisa
                glColor3f(cor.x, cor.y, cor.z)
                glTexCoord2f(*tex_coord3)
                glVertex3f(v3.x, v3.y, v3.z)
                
                cor = shading(v4, n4, self) * self.camisa
                glColor3f(cor.x, cor.y, cor.z)
    ...
    def desenha_tampa(self, raio, z, divisao):
        ...
        cor = shading(glm.vec3(0, 0, z), normal, self) # Calculando a iluminação
        ...
        for i in range(divisao + 1):
            ...
            cor = shading(v, normal, self) # Calculando a iluminação
            glColor3f(cor.x, cor.y, cor.z) # Cor do vertice
